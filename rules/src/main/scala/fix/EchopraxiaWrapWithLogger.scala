...
class EchopraxiaWrapMethodWithLogger(config: EchopraxiaWrapMethodWithLogger.Config) extends SemanticRule("EchopraxiaWrapMethodWithLogger") {
  ...
  override def withConfiguration(config: Configuration): Configured[Rule] =
    config.conf.getOrElse("EchopraxiaWrapMethodWithLogger")(this.config)
        ...
        if (isValidAccessModifier(mods)) {
          body match {
            ...
            case block @ Term.Block(statements) =>
              println(methodName.pos.formatMessage("info", s"Adding $add to `${methodName}`"))
              Patch.addLeft(block, s"$add ").atomic
            case other =>
              println(methodName.pos.formatMessage("info", s"Adding $add block to `${methodName}`"))
  ...
  private def isValidAccessModifier(mods: List[Mod]): Boolean = {
    methodAccess match {
      case "public" =>
        ...
        ! mods.exists(m => m.isInstanceOf[Mod.Private] || m.isInstanceOf[Mod.Protected])
      case "protected" =>
